...
### 2.1 AlDahoul et al., 2024 (arXiv 2410.24148) — SOTA publicado
...
➡️ **O paper-pai do dataset NÃO publica número para race 7-class in-domain.** A SOTA real surgiu em AlDahoul et al. 2024.

## 3. FineFACE (Manzoor & Rattani, 2024, arXiv 2408.16881) — NÃO é race classifier
...
### 4.0 Comparabilidade arquitetural (por que AlDahoul ResNet-34 é a referência primária)
...
| **AlDahoul ResNet-34** | **CNN puro discriminativo** | **~21M** | **ImageNet-1k** |
...
|---|---|---|---|---|---|
| **FaceScanPaliGemma** (AlDahoul et al. 2024, SOTA) | race 7-class | split oficial, imbalance natural | **0.757** | **0.750** | — |
| **YOLO11x** (community Anzhc) | race 7-class | split oficial, padding 0.25 | 0.735 | — | — |
| FairFace ResNet-34 baseline (re-impl. AlDahoul et al. 2024) | race 7-class | mesmo setup | 0.720 | — | — |
...
Após o anchor 🅔 (protocolo AlDahoul et al. completo) e a auditoria empírica
...
| **HPO da recipe não declarado por AlDahoul et al.** | ❌ irrecuperável | atribuição honesta do gap residual |
...
honesta: **HPO da recipe realizada por AlDahoul et al. e não publicada
integralmente**, fora do escopo desta dissertação.

### 4.2 Cruzamento de coerência (atualizado)

- AlDahoul ResNet-34 (mesma rede do paper-pai) entrega 0.720 sob seu
  protocolo.
- Nosso ConvNeXt-T (rede mais moderna), sob protocolo **idêntico** ao
  AlDahoul et al., entrega 0.706.
...
- **Achado adicional crucial:** nossa razão de disparidade (IR=1.541)
  sob 🅔 é a melhor de todos os pipelines do projeto. AlDahoul et al. não
...
A SOTA real (AlDahoul et al. 2024) **existe e foi mapeada**. O que **não existe** é uma SOTA para a pergunta que rodamos:
...
| 🅔 Anchor AlDahoul-protocol (controle + ConvNeXt) | ✅ **executado** | ConvNeXt F1=0.703±0.005, IR=1.541±0.044 (3.5σ + 1.8σ) |
...
| Dimensão | F1-F5 + Fator 5 (todos) | Anchor 🅓 (raw-data) | FairFace paper (2021) | FineFACE (2024) | AlDahoul et al. (2024) |
|---|---|---|---|---|---|
...
2. **AlDahoul et al. 2024 é o único trabalho na mesma tarefa exata** (race 7-class, padding presumível 0.25, split oficial). É a única comparação numerical legítima.
...
> em 7 categorias no FairFace no-domain: AlDahoul et al., 2024
...
> disparidade é a melhor de todos os pipelines do projeto, e AlDahoul et al.
...
> 7σ + 3σ, sem subamostragem 1.5σ + 0.7σ, AlDahoul-protocol
...
(dp=0.005). AlDahoul et al. 2024 não usou ensemble/TTA/calibração —
...
> (AlDahoul et al. 2024) e comparação simétrica single-run-vs-single-run,
> nosso ConvNeXt-T 🅔 entrega acurácia=0.7115 / F1=0.7083 / IR=1.496 —
> **−0.85pp do baseline AlDahoul ResNet-34 (0.720), dentro de 1.7σ da
...
> como reivindicação de superação direta (AlDahoul et al. não usou essas
