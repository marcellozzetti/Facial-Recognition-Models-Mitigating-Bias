...
def _cfg(backbone_arch: str, seed: int) -> dict:
    return {
        "model": {
            "name": "LResNet50E_IR",  # legacy mlflow tag (alias); arch below
            "backbone_arch": backbone_arch,  # the factor under study
            "pretrained": True,
            "num_classes": 7,
            "dropout": 0.2,
            "head": "linear",
...
def main() -> int:
    ...
    written = []
    for arm_key, arch in ARMS.items():
        for seed in SEEDS:
            cfg = _cfg(arch, seed)
            fname = f"exp_f5_{arm_key}_s{seed:02d}.yaml"
            header = (
                f"# Factor 5 (backbone) — {arch} seed={seed}\n"
