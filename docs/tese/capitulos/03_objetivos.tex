...
\item[C4.] Triangulação de métricas multi-classe --- \emph{Disparity Ratio} $+$ \emph{worst-class} F1 $+$ \emph{Equal Opportunity} por classe $+$ \emph{Equalized Odds} por classe ---, fundamentada no Teorema da Impossibilidade~\autocite{kleinberg2017inherent} e nas definições formais de \emph{fairness} de \citeauthor{hardt2016equality}~\autocite{hardt2016equality}. \textit{(OE5.)}
