...
\item \textbf{Etapa 3 --- Classificador racial com condicionamento}: treinamento de ConvNeXt-T \emph{fine-tuned} em FairFace \emph{train}, com camadas FiLM~\cite{perez2018film} por estágio recebendo como contexto o vetor MST produzido na Etapa~1.
...
\item \textbf{Etapa 6 --- Síntese decompositiva}: combinação dos resultados obtidos nas Etapas~2 e~5 para quantificar a contribuição relativa do componente fenotípico (irredutível, associado ao sobreposição MST intra-categoria) \emph{versus} componente algorítmico (mitigável) do erro de classificação Latinx.
\end{enumerate}
...
\item ConvNeXt-T puro, sem condicionamento, como controle arquitetural moderno;
...
\item \textbf{Comparabilidade com o \emph{baseline} canônico da literatura de \emph{fairness} facial.} A adoção do ConvNeXt-T como controle arquitetural moderno (Configuração A, Seção~\ref{sec:ablation}) permite isolar o efeito do condicionamento sem confundi-lo com ganhos derivados apenas da modernização do \emph{backbone} sobre o ResNet-34 usado por \citeonline{karkkainen2021fairface}.
\end{enumerate}

Como verificação básica arquitetural, o trabalho reporta explicitamente o desempenho do ConvNeXt-T puro (Configuração A) em separado do desempenho do ConvNeXt-T com FiLM (Configuração B), permitindo à banca examinadora avaliar de forma independente a contribuição do \emph{backbone} e a contribuição do mecanismo de condicionamento.
...
Todos os experimentos das três configurações (Seção~\ref{sec:ablation}) são conduzidos sob protocolo unificado:
...
\item \textbf{Biblioteca}: PyTorch 2.x sobre implementação de referência de ConvNeXt disponível em \emph{TorchVision}, com camadas FiLM implementadas como módulos PyTorch customizados.
\end{itemize}

Estes hiperparâmetros são fixados a priori e mantidos idênticos entre as três configurações (A, B, C) descritos na Seção~\ref{sec:ablation} e entre os seis \emph{baselines} (Seção~\ref{sec:baselines}), garantindo que diferenças de desempenho observadas sejam atribuíveis exclusivamente à variação do mecanismo de condicionamento ou da arquitetura do \emph{baseline}, e não a diferenças de otimização.


\section{\emph{Ablation} arquitetural}
\label{sec:ablation}

Esta pesquisa conduzirá \emph{ablation} arquitetural comparando três configurações do estágio de condicionamento, mantidos idênticos o \emph{backbone} (ConvNeXt-T), o dataset (FairFace) e o protocolo de avaliação:

\begin{description}[font=\bfseries\sffamily]
\item[Configuração A --- Baseline sem condicionamento]: ConvNeXt-T \emph{fine-tuned} sem qualquer sinal auxiliar, como controle arquitetural.
\item[Configuração B --- ConvNeXt-T com FiLM (proposta principal)]: sinal condicionante é o vetor MST 10-dim produzido pelo classificador da Etapa~1. Corresponde ao teste da hipótese central de que o condicionamento arquitetural por tom de pele reduz a disparidade demográfica sem sacrificar acurácia agregada. Variantes de implementação das funções $f_\gamma$ e $f_\beta$ (arquitetura das MLPs geradoras, presença ou não de portas multiplicativas) são exploradas como escolhas empíricas durante a fase experimental, sem alterar a definição da configuração.
\item[Configuração C --- ConvNeXt-T com FiLM sobre \emph{embedding} CLIP-\emph{text}]: alternativa em que o sinal condicionante é um \emph{embedding} semântico 512-dim produzido por CLIP-\emph{text}~\cite{luo2024fairclip} em vez do vetor MST discreto. O \emph{prompt} adotado é a média de \emph{embeddings} de dez \emph{templates} equivalentes na forma \emph{``a photo of a person with Monk Skin Tone $k$''}, com $k \in \{1, \ldots, 10\}$, ponderados pela probabilidade \emph{softmax} de cada classe MST produzida pelo classificador da Etapa~1. Esta estratégia de \emph{prompt ensembling}, alinhada às práticas de \citeonline{dehdashtian2024fairerclip}, evita a sensibilidade do CLIP a formulações isoladas e produz sinal condicionante mais robusto que \emph{prompts} estáticos únicos.
\end{description}

A Figura~\ref{fig:configuracoes-abcd} compara visualmente as três configurações, evidenciando a dimensão variada (natureza do sinal condicionante) e as dimensões controladas (\emph{backbone}, dataset, protocolo de avaliação).

\begin{figure}[htbp]
\centering
\caption{Três configurações comparativas do estudo de \emph{ablation} arquitetural.}
\centering
