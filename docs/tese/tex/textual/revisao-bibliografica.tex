...
\section{Alternativas de condicionamento}
\label{sec:rl_conditioning_alt}

A escolha do mecanismo FiLM como linha principal desta pesquisa se fundamenta na avaliação sistemática de oito alternativas de condicionamento disponíveis na literatura. A Tabela~\ref{tab:cond_alternativas} sintetiza essa análise considerando o cenário específico desta pesquisa: sinal de condicionamento vetorial de baixa dimensionalidade (vetor Monk Skin Tone de dez classes na saída \emph{softmax} do classificador da Etapa~1); \emph{backbone} moderno baseado em convoluções (ConvNeXt-T, com 28 milhões de parâmetros); exigência de interpretabilidade dos parâmetros de modulação por canal para análise posterior; e a lacuna científica quanto ao uso de condicionamento arquitetural em \emph{fairness} facial multi-classe, sintetizada na Seção~\ref{sec:rl_gaps}.
