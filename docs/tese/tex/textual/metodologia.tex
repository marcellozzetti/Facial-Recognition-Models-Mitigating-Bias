...
Esta pesquisa conduzirá \emph{ablation} arquitetural comparando três configurações do estágio de \emph{conditioning}, mantidos idênticos o \emph{backbone} (ConvNeXt-T), o dataset (FairFace) e o protocolo de avaliação:

\begin{description}[font=\bfseries\sffamily]
\item[Configuração A --- Baseline sem \emph{conditioning}]: ConvNeXt-T \emph{fine-tuned} sem qualquer sinal auxiliar, como controle arquitetural.
\item[Configuração B --- ConvNeXt-T com FiLM (proposta principal)]: sinal condicionante é o vetor MST 10-dim produzido pelo classificador da Etapa~1. Corresponde ao teste da hipótese central de que o \emph{conditioning} arquitetural por tom de pele reduz a disparidade demográfica sem sacrificar acurácia agregada. Variantes de implementação das funções $f_\gamma$ e $f_\beta$ (arquitetura das MLPs geradoras, presença ou não de portas multiplicativas) são exploradas como escolhas empíricas durante a fase experimental, sem alterar a definição da configuração.
